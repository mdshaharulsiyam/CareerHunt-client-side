...
const ApplicationUpdate = ({ defaultcategory, experience, title, id, refetch, setShowForm }) => {
  ...
  const onSubmit = (data) => {
    ...
    axiorequest.patch(`/teacthersaplication?id=${id}&useremail=${currentUser?.useremail}`, applicatioData)
      .then((res) => {
        if (res.data.success) {
          setloading(false)
          setShowForm(false)
          ...
          Swal.fire({
            ...
            footer: 'unable to updated application'
          });
          setloading(false)
        }
      })
  ...
  return (
    <div className="relative">
      ...
      <h3 className="text-4xl pt-12 text-center font-bold">update request</h3>
      {
      ...
      <form className="max-w-2xl mx-auto pb-12" onSubmit={handleSubmit(onSubmit)}>
        ...
        <input className="block outline-none border-b-2 w-full mx-auto p-2 pl-0 border-b-gray-400 my-2" type="text" defaultValue={title} placeholder="Title" {...register("title", { required: true })} />
        ...
        <select className="block outline-none border-b-2 w-full mx-auto p-2 pl-0 border-b-gray-400 my-2" {...register("experience", { required: true })}>
          ...
          {
            skillOptions.map(item => <option key={item} selected={item === experience} value={item}>{item}</option>)
          }
        </select>
        ...
        <select className="block outline-none border-b-2 w-full mx-auto p-2 pl-0 border-b-gray-400 my-2" {...register("category", { required: true })}>
          {
            category.map(item => (
              <option key={item} value={item} selected={item === defaultcategory}>
                {item}
              </option>
            ))
        ...
        <button className="w-full bg-red-600 cursor-pointer rounded-lg text-white py-2 hover:bg-red-900 transition-all" type="submit">{loading ? <span className="loading loading-bars loading-xs"></span> : 'update application'}</button>
      </form>
    </div>
  )
}
